...
def load_file_and_transmit_appearances(robot_id_, filepath, message_rate_=10000):
    ...
    for appearance in robot.links[0].appearances:
        ...
        whereis_message.robot_id     = robot_id_
        ...
        whereis_message.rate = message_rate_
...
if __name__ == "__main__":
    ...
    if len(args) > 3:
        message_rate_ = int(args[3])   
        print "arg 3: message_rate_ = %d" % message_rate_
    else:
        message_rate_ = 10000
        print "no-arg message_rate_ = %d" % message_rate_
    configurate_task_publishers()    
    # localise_from_feature_from_visionsources(robot_id_)
    load_file_and_transmit_appearances(robot_id_, filepath_, message_rate_)
