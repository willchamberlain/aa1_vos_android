...
def call_services():
    ...
    try:
        initialpose_proxy = rospy.ServiceProxy('/Pioneer2_vc_initialpose', InitialPoseToRobot)
    ...
    try:
        base_pose_proxy = rospy.ServiceProxy('/Pioneer2_vc_base_pose', PoseToRobot)
        resp = base_pose_proxy(odom) 
    except rospy.ServiceException, e:
        print '/Pioneer2_vc_base_pose service call failed: %s'%e    
